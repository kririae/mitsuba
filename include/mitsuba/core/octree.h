...
class LockFreeList {
 public:
  struct ListItem {
    ...
    inline ListItem(const T &value) : value(value), next(NULL) {}
  };
  ...
  inline const ListItem *head() const { return m_head; }
...
class StaticOctree {
 public:
   ...
   */
  inline StaticOctree(const AABB &aabb, uint32_t maxDepth = 24,
                      uint32_t maxItems = 8)
      : m_aabb(aabb),
        m_maxDepth(maxDepth),
        m_maxItems(maxItems),
  ...
  void build() {
    ...
    for (uint32_t i = 0; i < m_items.size(); ++i) perm[i] = i;
 ...
 protected:
  ...
  inline AABB childBounds(int child, const AABB &nodeAABB,
...
/**
 * \brief Generic multiple-reference octree with support for parallel dynamic
 * updates
 *
...
class DynamicOctree {
 public:
  ...
  /// Execute <tt>functor.operator()</tt> on all records, which potentially
  /// overlap \c p
  template <typename Functor>
  inline void lookup(const Point &p, Functor &functor) const {
    if (!m_aabb.contains(p)) return;
  ...
  /// overlap \c bsphere
  template <typename Functor>
  inline void searchSphere(const BSphere &sphere, Functor &functor) {
    if (!m_aabb.overlaps(sphere)) return;
 ...
 private:
  ...
  void insert(OctreeNode *node, const AABB &nodeAABB, const Item &value,
    ...
    for (int child = 0; child < 8; ++child) {
      ...
      if (!node->children[child]) {
        ...
        if (!atomicCompareAndExchangePtr<OctreeNode>(&node->children[child],
                                                     newNode, NULL))
          delete newNode;
      }
      const AABB childAABB(childBounds(child, nodeAABB, center));
      insert(node->children[child], childAABB, value, coverage, diag2,
             depth + 1);
    }
  }
  ...
  template <typename Functor>
  inline void lookup(const OctreeNode *node, const AABB &nodeAABB,
                     const Point &p, Functor &functor) const {
    const Point center = nodeAABB.getCenter();
    ...
    int child = (p.x > center.x ? 4 : 0) + (p.y > center.y ? 2 : 0) +
                (p.z > center.z ? 1 : 0);
  ...
  template <typename Functor>
  inline void searchSphere(OctreeNode *node, const AABB &nodeAABB,
                           const BSphere &sphere, Functor &functor) {
    const Point center = nodeAABB.getCenter();
...
class UnsafeLockFreeList {
 public:
  ...
  inline ListItem *head() { return m_head; }
...
/**
 * \brief This octree is derived from the previous octree, which unlocks
 * modification This class is currently used to implement path guiding.
 *
 * \ingroup libcore
 */
template <typename Item>
class UnsafeDynamicOctree {
 public:
  /**
   * \brief Create a new octree
   *
  ...
  /// overlap \c p
  template <typename Functor>
  inline void lookup(const Point &p, const Functor &functor) const {
    if (!m_aabb.contains(p)) return;
    lookup(&m_root, m_aabb, p, functor);
  }
  ...
  /// overlap \c bsphere
  template <typename Functor>
  inline void searchSphere(const BSphere &sphere, const Functor &functor) {
    if (!m_aabb.overlaps(sphere)) return;
    searchSphere(&m_root, m_aabb, sphere, functor);
  }
 ...
 private:
  ...
  template <typename Functor>
  inline void lookup(const OctreeNode *node, const AABB &nodeAABB,
                     const Point &p, const Functor &functor) const {
    const Point center = nodeAABB.getCenter();
  ...
  template <typename Functor>
  inline void searchSphere(OctreeNode *node, const AABB &nodeAABB,
                           const BSphere &sphere, const Functor &functor) {
    const Point center = nodeAABB.getCenter();
