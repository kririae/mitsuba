...
struct MTS_EXPORT_CORE SHVector {
 public:
  ...
  inline SHVector(int bands) : m_bands(bands), m_coeffs(bands * bands) {
    clear();
  }
  ...
  /// Copy constructor
  inline SHVector(const SHVector &v)
      : m_bands(v.m_bands), m_coeffs(v.m_coeffs) {}

  /// Return the number of stored SH coefficient bands
  inline int getBands() const { return m_bands; }
  ...
  inline Float energy(int band) const {
    Float result = 0;
    for (int m = -band; m <= band; ++m) result += std::abs(operator()(band, m));
    return result;
  }
  ...
  inline void clear() { m_coeffs.setZero(); }
  ...
  inline Float &operator()(int l, int m) { return m_coeffs[l * (l + 1) + m]; }
  ...
  /// Equality comparison operator
  inline bool operator!=(const SHVector &v) const { return !operator==(v); }
  ...
  /// Project val into the spherical harmonics
  // TODO: might be problematic in case of normalization
  void addDelta(Float value, Float theta, Float phi) {
    /* Initialize these basic values */
    ...
    for (int l = 0; l < m_bands; ++l) {
      ...
      operator()(l, 0) +=
          value * legendreP(l, 0, cosTheta) * normalization(l, 0);
    }
  }

  /// Project the given function onto a SH basis (using a 2D composite Simpson's
  /// rule)
  ...
  void project(const Functor &f, int res = 32) {
    ...
    for (int l = 0; l < m_bands; ++l)
      for (int m = -l; m <= l; ++m) operator()(l, m) = 0;
    ...
    for (int i = 0; i <= res; ++i) {
      ...
      if (i == 0 || i == res) weightExt = 1.0f;

      for (int j = 0; j <= res * 2; ++j) {
        Float phi       = hInt * j;
        Float weightInt = (j & 1) ? 4.0f : 2.0f;
        if (j == 0 || j == 2 * res) weightInt = 1.0f;
    ...
    for (int l = 0; l < m_bands; ++l)
      for (int m = -l; m <= l; ++m) operator()(l, m) *= hExt * hInt / 9;
  }
  ...
  /**
   * \brief Recursively computes rotation matrices for each band of SH
   * coefficients.
   *
   * Based on 'Rotation Matrices for Real Spherical Harmonics. Direct
   * Determination by Recursion' by Ivanic and Ruedenberg. The implemented
   * tables follow the notation in 'Spherical Harmonic Lighting: The Gritty
 ...
 protected:
  /// Helper function for rotation() -- computes a diagonal block based on the
  /// previous level
  static void rotationBlock(const SHRotation::Matrix &M1,
...
class MTS_EXPORT_CORE SHSampler : public Object {
 ...
 protected:
  ...
  inline Float lookupIntegral(int depth, int zBlock, int phiBlock, int l,
                              int m) const {
    return -m_phiMap[depth][phiBlock][P(m)] *
           m_legendreMap[depth][zBlock][I(l, std::abs(m))];
  }
