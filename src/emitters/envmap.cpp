 ...
 *        Preserve generated MIP map data in a cache file? This will cause a
 * file named \emph{filename}\code{.mip} to be created. \default{automatic---use
 * caching for images larger than 1M pixels.}
 *     }
 ...
 * which is a type of light source that is well-suited for representing
 * ``natural'' illumination. Many images in this document are made using the
 * environment map shown in (a).
 *
 ...
 * The image could either be a processed photograph or a rendering made using
 * the \pluginref{spherical} sensor. The direction conventions of this
 * transformation are shown in (b). The plugin can work with all types of images
 * that are natively supported by Mitsuba (i.e. JPEG, PNG, OpenEXR, RGBE, TGA,
 * and BMP). In practice, a good environment map will contain high-dynamic range
 * data that can only be represented using the OpenEXR or RGBE file formats.
 ...
 * significantly accelerates the loading times of subsequent renderings. When
 * this is not desired, specify \code{cache=false} to the plugin.
 */
class EnvironmentMap : public Emitter {
 public:
  ...
  EnvironmentMap(const Properties &props)
    ...
    if (props.hasProperty("bitmap")) {
      ...
      Log(EInfo, "Loading environment map \"%s\"",
          m_filename.filename().string().c_str());
      if (!fs::exists(m_filename))
        Log(EError, "Environment map file \"%s\" could not be found!",
            m_filename.string().c_str());

      boost::system::error_code ec;
      timestamp = (uint64_t)fs::last_write_time(m_filename, ec);
      if (ec.value())
        Log(EError, "Could not determine modification time of \"%s\"!",
            m_filename.string().c_str());
    ...
    if (tryReuseCache &&
        MIPMap::validateCacheFile(cacheFile, timestamp, ENVMAP_PIXELFORMAT,
                                  ReconstructionFilter::ERepeat,
                                  ReconstructionFilter::EClamp, filterType,
                                  m_gamma)) {
      ...
      if (bitmap == NULL) {
        ...
        if (m_gamma != 0) bitmap->setGamma(m_gamma);
        Log(EDebug, "Loaded \"%s\" in %i ms",
            m_filename.filename().string().c_str(), timer->getMilliseconds());
      }

      if (std::max(bitmap->getWidth(), bitmap->getHeight()) > 0xFFFF)
        Log(EError,
            "Environment maps images must be smaller than 65536 "
      ...
      bool createCache =
          !cacheFile.empty() &&
          props.getBoolean(
              "cache", bitmap->getSize().x * bitmap->getSize().y > 1024 * 1024);

      m_mipmap = new MIPMap(bitmap, ENVMAP_PIXELFORMAT, Bitmap::EFloat, rfilter,
                            ReconstructionFilter::ERepeat,
                            ReconstructionFilter::EClamp, filterType,
                            maxAnisotropy, createCache ? cacheFile : fs::path(),
                            timestamp, std::numeric_limits<Float>::infinity(),
                            Spectrum::EIlluminant);
    }

    if (props.hasProperty("intensityScale"))
      Log(EError,
          "The 'intensityScale' parameter has been deprecated and is now "
          "called scale.");
  ...
  EnvironmentMap(Stream *stream, InstanceManager *manager)
    ...
    Log(EDebug, "Unserializing texture \"%s\"",
        m_filename.filename().string().c_str());
    ...
    m_mipmap = new MIPMap(bitmap, ENVMAP_PIXELFORMAT, Bitmap::EFloat, rfilter,
                          ReconstructionFilter::ERepeat,
                          ReconstructionFilter::EClamp, EEWA, 10.0f, fs::path(),
  ...
  virtual ~EnvironmentMap() {
    ...
    if (m_cdfRows) delete[] m_cdfRows;
    if (m_cdfCols) delete[] m_cdfCols;
    if (m_rowWeights) delete[] m_rowWeights;
  }
  ...
  void configure() {
    ...
    if (!m_rowWeights) {
      ...
      Log(EInfo,
          "Precomputing data structures for environment map sampling (%s)",
      ...
      if (rowSum == 0)
        Log(EError,
            "The environment map is completely black -- this is not allowed.");
      else if (!std::isfinite(rowSum))
        Log(EError,
            "The environment map contains an invalid floating"
            " point value (nan/inf) -- giving up.");

      m_normalization =
          1.0f / (rowSum * (2 * M_PI / m_size.x) * (M_PI / m_size.y));
    ...
    Float surfaceArea =
        4 * M_PI * m_sceneBSphere.radius * m_sceneBSphere.radius;
  ...
  ref<Shape> createShape(const Scene *scene) {
    ...
    Transform trafo = Transform::translate(Vector(m_sceneBSphere.center)) *
    ...
    Shape *shape = static_cast<Shape *>(
        PluginManager::getInstance()->createObject(MTS_CLASS(Shape), props));
    shape->addChild(this);
    shape->configure();
  ...
  bool fillDirectSamplingRecord(DirectSamplingRecord &dRec,
                                const Ray            &ray) const {
    Float nearT, farT;

    if (!m_sceneBSphere.rayIntersect(ray, nearT, farT) || nearT > 0 ||
        farT < 0) {
  ...
  Spectrum evalEnvironment(const RayDifferential &ray) const {
    ...
    Point2 uv(std::atan2(v.x, -v.z) * INV_TWOPI, math::safe_acos(v.y) * INV_PI);
   ...
   * components separately (in that order!). As nice as this is for
   * bidirectional techniques, it is unfortunately very difficult to reconcile
   * with the default environment map strategy of importance sampling a
   * direction *first* from the environment, followed by (*second*) uniformly
   * picking a position on a hyperplane perpendicular to that direction.
   *
   * Therefore, the following compromise is implemented:
   * 1. sampleArea(): uniformly sample a position on the scene's bounding sphere
   * 2. sampleDirection(): importance sample a direction from the environment
   * map
   *
   * This is not that great, as many useless samples will be generated. Note
   * however that rendering scenes by tracing from the environment side is
   * usually a terrible strategy in any case, so at least not much is lost by
   * making it worse.
   *
   * The direct illumination sampling code (\ref sampleDirect()), and the
   * combined position + direction code (\ref sampleRay) both does the right
   * thing and are not affected by any of this. Many of the bidirectional
   * rendering algorithms in Mitsuba can make use of direct illumination
   * sampling strategies.
   */
  ...
  Spectrum sampleDirect(DirectSamplingRecord &dRec,
    ...
    Float nearT, farT;
    if (value.isZero() || pdf == 0 ||
        !m_sceneBSphere.rayIntersect(ray, nearT, farT) || nearT >= 0 ||
        farT <= 0) {
      dRec.pdf = 0.0f;
      return Spectrum(0.0f);
    }
  ...
  void internalSampleDirection(Point2 sample, Vector &d, Spectrum &value,
                               Float &pdf) const {
    /* Sample a discrete pixel position */
    uint32_t row = sampleReuse(m_cdfRows, m_size.y, sample.y),
             col = sampleReuse(m_cdfCols + row * (m_size.x + 1), m_size.x,
                               sample.x);
    ...
    Float dx1 = pos.x - xPos, dx2 = 1.0f - dx1, dy1 = pos.y - yPos,
          dy2 = 1.0f - dy1;
    ...
    pdf   = (value1.getLuminance() *
               m_rowWeights[math::clamp(yPos, 0, m_size.y - 1)] +
           value2.getLuminance() *
               m_rowWeights[math::clamp(yPos + 1, 0, m_size.y - 1)]) *
          m_normalization;
  ...
  /// Helper function that computes the solid angle density of \ref
  /// internalSampleDirection()
  Float internalPdfDirection(const Vector &d) const {
    /* Convert to latitude-longitude texture coordinates */
    Point2 uv(std::atan2(d.x, -d.z) * INV_TWOPI, math::safe_acos(d.y) * INV_PI);
    ...
    Float dx1 = u - xPos, dx2 = 1.0f - dx1, dy1 = v - yPos, dy2 = 1.0f - dy1;
  ...
  std::string toString() const {
    ...
    oss << "EnvironmentMap[" << endl
        ...
        << "  worldTransform = " << indent(m_worldTransform.toString()) << ","
        << endl
 ...
 private:
  ...
  inline uint32_t sampleReuse(float *cdf, uint32_t size, Float &sample) const {
    ...
    uint32_t index =
        std::min((uint32_t)std::max((ptrdiff_t)0, entry - cdf - 1), size - 1);
    sample =
        (sample - (Float)cdf[index]) / (Float)(cdf[index + 1] - cdf[index]);
    return index;
  }
...
class EnvironmentMapShader : public Shader {
 public:
  EnvironmentMapShader(Renderer *renderer, const fs::path &filename,
                       ref<Bitmap> bitmap, const Transform &worldToEmitter,
                       Float scale)
      : Shader(renderer, EEmitterShader) {
    std::string name = filename.filename().string();
    if (name.empty()) name = "Environment map";
    ...
    m_scale                     = scale;
    m_useCustomTextureFiltering = renderer->getCapabilities()->isSupported(
        RendererCapabilities::ECustomTextureFiltering);
  }

  void cleanup(Renderer *renderer) { m_gpuTexture->cleanup(); }

  void resolve(const GPUProgram *program, const std::string &evalName,
               std::vector<int> &parameterIDs) const {
    parameterIDs.push_back(
        program->getParameterID(evalName + "_texture", false));
    parameterIDs.push_back(
        program->getParameterID(evalName + "_worldToEmitter", false));
  ...
  void generateCode(std::ostringstream &oss, const std::string &evalName,
    ...
    oss << "uniform sampler2D " << evalName << "_texture;" << endl
        ...
        << "   const float inv_pi = 0.318309886183791, inv_twopi = "
           "0.159154943091895;"
        << endl
        << "   vec3 v = normalize((" << evalName
        << "_worldToEmitter * vec4(v_, 0.0)).xyz);" << endl
        << "   vec2 coords = vec2(atan(v.x, -v.z) * inv_twopi, acos(v.y) * "
           "inv_pi);"
        << endl;

    if (m_useCustomTextureFiltering) {
      oss << "   /* Manually compute derivatives to work around "
             "discontinuities */"
          << endl
          << "   vec3 dvdx = dFdx(v), dvdy = dFdy(v);" << endl
          << "   float t1 = inv_twopi / (v.x*v.x+v.z*v.z)," << endl
          << "         t2 = -inv_pi / sqrt(1.0-v.y*v.y);" << endl
          << "   vec2 dudx = vec2(t1 * (dvdx.z*v.x - dvdx.x*v.z), t2 * dvdx.y),"
          << endl
          << "        dudy = vec2(t1 * (dvdy.z*v.x - dvdy.x*v.z), t2 * dvdy.y);"
          << endl
          << "   if (abs(v.y) > 0.99) { dudx = dudy = vec2(0.0); /* Don't blur "
             "over vertical edges */ }"
          << endl
          << "   return texture2DGrad(" << evalName
          << "_texture, coords, dudx, dudy).rgb * " << evalName << "_scale;"
          << endl;
    } else {
      oss << "   return texture2D(" << evalName << "_texture, coords).rgb * "
          << evalName << "_scale;" << endl;
    }
  ...
  void unbind() const { m_gpuTexture->unbind(); }
