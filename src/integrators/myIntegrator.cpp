...
class MyIntegrator : public SamplingIntegrator {
 public:
  ...
  // preprocess to get the max distance for normalization
  bool preprocess(const Scene *scene, RenderQueue *queue, const RenderJob *job,
                  int sceneResID, int cameraResID, int samplerResID) {
    SamplingIntegrator::preprocess(scene, queue, job, sceneResID, cameraResID,
                                   samplerResID);
...
MTS_EXPORT_PLUGIN(MyIntegrator, "My integrator");
MTS_NAMESPACE_END